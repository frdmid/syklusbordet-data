...
MIN_AAR = 4
STRESS_AAR = 5          # verste driftskontantstrom i dette vinduet er stresstilfellet
...
for tk in sorted(AKSJER):
    ...
    drift = serier["drift"]
    stress = float(drift.iloc[-STRESS_AAR:].min())
    ...
    # C1: kvartaler kontantene holder hvis driften gaar like daarlig som paa
    # sitt verste de siste fem aarene, med renter paa toppen.
    brenn = max(0.0, -stress) + (rente or 0.0)
    kvartaler = None if brenn <= 0 else round(kont / (brenn / 4), 1)
    ...
    if kvartaler is None:
        port, hvorfor = "aapen", "positiv drift selv i verste aar"
    ...
    SELSKAP[tk] = {"ticker": tk, "navn": AKSJER[tk]["navn"], "bors": AKSJER[tk]["bors"],
                   ...
                   "kvartaler": kvartaler, "netto_gjeld_ek": ngek, "rentedekning": dekning,
                   "kontanter_musd": round(kont / 1e6, 0),
                   "verste_drift_musd": round(stress / 1e6, 0),
                   "aar": [int(a) for a in drift.index[-STRESS_AAR:]],
                   "begreper": valgt}
    print(f"   {tk:12s} {port:7s} {hvorfor[:34]:36s} netto gjeld/EK {str(ngek):>6}  "
          f"[{valgt.get('drift','')[:30]}]")
    time.sleep(0.3)
    ...
    body = {"message": "overlevelsesport C", "branch": BRANCH,
            "content": base64.b64encode(json.dumps(
                {"oppdatert": str(pd.Timestamp.utcnow())[:19],
                 "terskel_kvartaler": PORT_KVARTALER, "stressvindu_aar": STRESS_AAR,
                 "selskaper": SELSKAP, "segmenter": SEG,
                 "utenfor_sec": utenfor,
                 "merknad": ("C1 er hvor mange kvartaler kontantbeholdningen holder hvis "
                             "driftskontantstrommen blir like daarlig som sitt verste i "
                             "vinduet, med renter paa toppen. Under aatte kvartaler regnes "
